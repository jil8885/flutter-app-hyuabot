...
class BusLaneCycle extends CustomPainter{
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(int i=0; i<5; i++){
      canvas.drawCircle(Offset(40 + (size.width - 80) / 4 * i, size.height / 2.5), 3, circle);
      TextSpan sp = TextSpan(style: TextStyle(color: Colors.white), text: stopList[i]);
      TextPainter tp = TextPainter(text: sp, textDirection: TextDirection.ltr);
      tp.layout();
      tp.paint(canvas, Offset(20 + (size.width - 80) / 4 * i, size.height / 2.5 - 40));
    }
...
class BusLaneDirectStation extends CustomPainter{
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(int i=0; i<4; i++){
      canvas.drawCircle(Offset(40 + (size.width - 80) / 3 * i, size.height / 2.5), 3, circle);
      TextSpan sp = TextSpan(style: TextStyle(color: Colors.white), text: stopList[i]);
      TextPainter tp = TextPainter(text: sp, textDirection: TextDirection.ltr);
      tp.layout();
      tp.paint(canvas, Offset(20 + (size.width - 80) / 3 * i, size.height / 2.5 - 40));
    }
